...
export function TopDomains(props: { range: () => string; client: () => string }) {
  const filterKey = () => `${props.range()}|${props.client()}`;
  const state = createPolling(
    filterKey,
    (key) => {
      const [r, c] = key.split("|");
      return api.topDomains(r, c || undefined, 15);
    },
    10000
  );
          ...
          <For each={state().data!}>
            {(d, i) => (
              <div class="bar-row" style={{ "animation-delay": `${0.28 + i() * 0.04}s` }}>
                ...
                <div class="bar-track">
                  <div class={`bar-fill${d.total_hits / maxHits() > 0.8 ? " danger" : ""}`} style={{ width: `${(d.total_hits / maxHits()) * 100}%` }} />
                </div>
